...
class CollectViewModel {
    ...
    lazy var wyVariable: BehaviorRelay<[ModelSection]> = {
...
extension CollectViewModel {
    fileprivate func getListData(_ text:String) -> [ModelSection] {
        ...
        apiHelper.fetchBasicInfo().subscribe(onNext: { [unowned self] (data) in
            if text.count > 0 {
                ...
                //修改value值,重新发出事件
                let section = ModelSection.init(items: datas!)
                self.wyVariable.accept([section])
            } else {
               let section = ModelSection.init(items: data.list!)
               self.wyVariable.accept([section])
            }
        }, onError: { (error) in
                print("error",error)
